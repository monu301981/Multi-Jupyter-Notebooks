...
docker build -t my-jupyter-cpp ./
docker run -p 10007:8888 -v %workspace%\%workfolder%:/home/jovyan my-jupyter-cpp

:: docker pull condaforge/miniforge3
:: docker run -it --rm -p 10006:8888 -v %workspace%\%workfolder%:/home/jovyan/work condaforge/miniforge3:latest /bin/bash -c "mamba create -n cling  -y && source activate cling && mamba install xeus-cling -c conda-forge  -y&& mamba install jupyterlab -c conda-forge  -y && jupyter lab --ip=0.0.0.0 --port=8888 --allow-root  --NotebookApp.token='' --notebook-dir='/home/jovyan/work'"
